  ...
  public ResponseEntity<?> registerUser(@Valid @RequestBody RegisterRequest registerRequest, HttpServletRequest request, @RequestHeader(value = "host") String domainName) {
    ...
    if (domain != null && domain.isEmailEnabled() && domain.isEmailValid()) {
      if (domain.isEmailRequiredEnabled()) {
        emailService.createEmailRegistration(domain, email, username, password, domainName, platform, regIp, promocodeName, refId);
        emailRequiredConfirm = true;
        registered = true;
      } else {
        user = userService.createUser(domain, email, username, password, domainName, platform, regIp, promocodeName, refId, false);
      ...
      if (adminEmailSettings.isEnabled() && adminEmailSettings.isValid()) {
        if (adminEmailSettings.isRequiredEnabled()) {
          emailService.createEmailRegistration(null, email, username, password, domainName, platform, regIp, promocodeName, refId);
          emailRequiredConfirm = true;
          registered = true;
        } else {
          user = userService.createUser(null, email, username, password, domainName, platform, regIp, promocodeName, refId, false);
    ...
    if (!registered) {
      user = userService.createUser(domain, email, username, password, domainName, platform, regIp, promocodeName, refId, true);
    }
