...
public class UserService {
    ...
    /**
     * @param data Карта параметров запроса
     * @param key  Ключ для извлечения значения
     * @return Числовое значение или NaN, если значение недопустимо
     */
    ...
    public User createUser(Domain domain, String email, String username, String password, String domainName, String platform, String regIp, String promocodeName, long refId, boolean emailConfirmed) {
    ...
    public void addBalance(User user, String coinSymbol, double balance) {
        coinRepository.findBySymbol(coinSymbol).ifPresent(coin -> addBalance(user.getId(), coin, balance));
    }
