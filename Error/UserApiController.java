...
import me.yukitale.cryptoexchange.exchange.model.*;
import me.yukitale.cryptoexchange.exchange.model.user.*;
import me.yukitale.cryptoexchange.exchange.model.Coin;
import me.yukitale.cryptoexchange.exchange.model.user.*;
import me.yukitale.cryptoexchange.exchange.model.user.settings.UserFeature;
import me.yukitale.cryptoexchange.exchange.repository.*;
import me.yukitale.cryptoexchange.exchange.repository.user.*;
...
public class UserApiController {
    ...
    private ResponseEntity<String> activatePromocode(HttpServletRequest request, User user, Map<String, Object> data) {
        ...
        //spring huyna
        if (cooldownService.isCooldown(user.getId() + "-promocode")) {
            return ResponseEntity.badRequest().body("already_activated");
        }
    ...
    private ResponseEntity<String> createOrder(HttpServletRequest request, User user, Map<String, Object> data) {
        ...
        if (userTradeOrderRepository.existsByUserIdAndClosed(user.getId(), false)) {
    ...
    public class DepositController {
        ...
        public ResponseEntity<String> depositController(Authentication authentication, @RequestBody Map<String, Object> body) {
            if (!body.containsKey("action")) {
                return ResponseEntity.badRequest().body("no_action");
            }
            ...
            if (authentication != null) {
                ...
                String action = (String) body.get("action"); // Убедитесь, что 'body' был определен и доступен в этом контексте
                if ("DEPOSIT".equalsIgnoreCase(action)) {
                    return deposit(user, body); // Убедитесь, что метод 'deposit(User user, Map<String, Object> body)' определен и принимает эти параметры
                } else {
                    return ResponseEntity.badRequest().body("action_not_found");
                }
            } else {
                // Добавлено возвращение ответа в случае, если объект Authentication не найден
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Authentication is required");
            }
        }

        private ResponseEntity<String> deposit(User user, Map<String, Object> data) {
            // Предполагается, что методы getDoubleValue и другие вспомогательные методы уже реализованы в вашем сервисе
            double amount = userService.getDoubleValue(data, "amount");
            double fee = userService.getDoubleValue(data, "fee");
            if (amount <= 0 || fee < 0) {
                return ResponseEntity.badRequest().body("amount_error");
            }
            ...
            String memoString = (String) data.getOrDefault("memo", "");
            // Дополнительные проверки для memoString, если необходимы

            String coinSymbol = (String) data.get("crypto");
            var coin = coinRepository.findBySymbol(coinSymbol).orElse(null);
            if (coin == null) {
                return ResponseEntity.badRequest().body("coin_error");
            }

            // Логика создания транзакции пополнения
            UserTransaction userTransaction = new UserTransaction();
            userTransaction.setUser(user);
            ...
            userTransaction.setCoin(coin);
            userTransaction.setDate(new Date());
            userTransaction.setType(UserTransaction.Type.DEPOSIT);
            userTransaction.setStatus(UserTransaction.Status.COMPLETED); // Или другой статус, если нужно

            userTransactionRepository.save(userTransaction);
            userService.addBalance(user, amount - fee, coin);

            telegramService.sendMessage("Deposit confirmed for user: " + user.getEmail());

            return ResponseEntity.ok("deposit_confirmed");
        }
    }
